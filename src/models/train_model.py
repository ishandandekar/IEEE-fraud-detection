...
fraud_detection_pipe = Pipeline(
    [
        (
            ...
            feature_preprocessors.MostFrequentImputer(features=config.impute_most_freq_cols),
        ),
        (
            "aggregate_high_cardinality_features",
            feature_preprocessors.AggregateCategorical(features=config.high_cardinality_cats),
        ),
        (
            "get_categorical_codes",
            feature_preprocessors.CategoryConverter(features=config.convert_to_category_codes),
        ),
        (
            "mean_imputer",
